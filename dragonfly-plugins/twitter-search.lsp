...
;; @version 0.50
;;  modded by cormullion 2010-01-13 15:15:58
;;  might have problems with parse-data - uses timeutilities.lsp? (old version for < 10.1.8, new for >= 10.1.8
...
(define (twitter-search keyword max-items)
  (let ((results "") (data '()))
    (set 'keyword (Request:utf8-urlencode keyword))
	(set 'xml (get-url (string "http://search.twitter.com/search.atom?rpp=" max-items {&q=} {%22} keyword)))
    ...
    ; print a graph
    (if (> (length data) 1) (Twitter-graph:twitter-graph data keyword))
    ; output any tweets as well
    (println results)
	)
)
